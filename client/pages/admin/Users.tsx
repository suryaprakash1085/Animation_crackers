...
export default function Users() {
  ...
  const [form, setForm] = useState<{ email: string; role: Role }>({ email: "", role: "customer" });
  ...
  const onEdit = (u: User) => { setForm({ email: u.email, role: u.role }); modal.openEdit(u); };
  const save = async () => {
    if (modal.item) {
      try {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Users" description="Registered users" />
      <TableFilters
        search={search}
      ...
      <FormModal
        ...
        title={modal.mode === "view" ? "View User" : "Edit User"}
        onSubmit={save}
